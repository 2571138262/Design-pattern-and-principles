...
##### 1、迭代器模式和访问者模式
###### 两者都是迭代的访问一个集合对象中的元素，
###### 不同的是在访问者模式中扩展开放的部分是作用于对象的操作上
###### 而迭代器模式中扩展开发的部分，是在集合对象的种类上，两者实现的方式上有很大的区别
...
### 策略模式 
#### 定义：定义了算法家族，分别封装起来，让他们之间可以狐仙替换，此模式让算法的变化不会影响到使用算法的用户。
#### if...else...的代码可以通过策略模式给消除掉
#### 类型：行为型
...
#### 策略模式 - 优点
##### 1、开闭原则
##### 2、可以避免使用多重条件转移语句 (if else  switch)
##### 3、提高算法的保密性和安全性
...
#### 策略模式 - 相关的模式
##### 策略模式 和 工厂模式(工厂方法和抽象工厂)
###### 策略模式是行为型的设计模式，工厂模式是创建型的设计模式
##### 策略模式 和 状态模式
###### 在使用策略模式的时候，客户端需要知道具体使用哪一个策略，
###### 在使用状态模式的时候客户端是不需要关心具体的状态的，这些状态会自动装换



### 解释器模式 
#### 定义：给定一个语言，定义它的文法(一种语法)的一种表示，并定义个解释器，这个解释器使用该表示来解释语言中的句子
#### 为了解释一种语言，而为语言创建的解释器 (Java, C# )

#### 类型 ： 行为型

#### 适用场景：
##### 1、某个特定类型问题发生频繁足够高
###### 例如：日常用脚本语言或者编程语言来处理一些日志的时候，
...
#### 解释器模式 - 相关的设计模式
##### 解释器模式 和 适配器模式
###### 适配器模式是不需要预先知道要适配的规则，解释器模式需要提前知道适配的规则
...
### 观察者模式
#### 定义： 定义了对象之前的一对多依赖，让多个观察者对象同时监听某一个主题对象，当主题对象发生变化时，它的所有依赖者(观察者)都会受到通知并更新
###### 例如；微信朋友圈评论，朋友圈信息就是主题对象，你就是观察者，当这条信息被评论之后，微信就会通知点赞的人(观察者)，不需要实时盯着这条消息，
###### 例如：京东上对商品进行关注以后，会有降价提醒。

#### 类型：行为型

#### 观察者模式 - 适用场景
##### 1、关联行为场景，建立一套触发机制
##### Observable（被观察者 目标对象）  Observer（观察者）
...
#### 备忘录模式 - 相关的设计模式
##### 1、备忘录模式 和 状态模式
###### 备忘录模式中是用实例来表示状态的，而在状态模式中则是用类来表示状态的

#### 源码中的运用
    源码中应用比较少的
    org.springframework.webflow 工作流 
    


### 命令模式 
#### 定义：将"请求"封装成对象，以便使用不同的请求  
#### 命令模式解决了应用程序中对象的职责以及他们之间的通行方式 (可以使发送者和接收者之间完全解耦)
#### 类型：行为型

#### 命令模式 - 适用场景
##### 1、请求调用者和请求接收者需要解耦，使得调用者和接收者不能交互
##### 2、需要抽象出等待执行的行为
...
#### 源码中的应用
    java.lang.Runnable 实现了Runnable的类，就相当于是一个具体的命令实现
...
### 中介者模式
#### 定义：定义了一个 封装一组对象如何交互 的对象
#### 通过使对象明确地相互引用来促进松耦合，并允许独立地改变他们的交互

#### 类型：行为型

#### 中介者模式 - 适用场景
##### 1、系统中对象之间存在复杂的引用关系，产生的相互依赖关系结构混乱且难以理解
##### 2、交互的公共行为，如果需要改变行为则可以增加新的中介者类(例如：聊天室)
...
#### 责任链模式 - 相关的设计模式
##### 责任链模式 和 状态模式
###### 在责任链模式当中，各个对象并不指定下一个处理对象者是谁，只有在客户端设定这个链条的顺序以及元素，直到被某个责任链元素处理或者整个链条结束
###### 状态模式是让每个状态对象知道自己下一个处理对象是谁，也就是说在编译时就设定好了
...
### 访问者模式
#### 定义：封装作用于某种数据结构(如List/Set/Map等)中的各元素的操作
#### 可以在不改变各元素的类的前提下，定义作用于这些元素的操作
##### 根据不同的 visitor 对相同的数据产生不同的操作行为
#### 类型：行为型

#### 访问者模式 - 适用场景
##### 1、一个数据结构如(List/Set/Map)包含很多类型对象
##### 2、数据结构与数据操作分离

#### 访问者模式 - 优点
##### 增加新的操作很容易，即增加一个新的访问者
###### 访问者模式通过将相关的行为集中到一个访问者对象中
#### 访问者模式 - 缺点
##### 1、增加新的数据结构困难
##### 2、具体元素变更比较麻烦
...
#### 访问者模式 - 相关设计模式
##### 访问者模式 和 迭代器模式
###### 共同点：都是在某种数据结构(List/Set/Map)上进行处理
###### 访问者模式主要用于对保存在数据结构中的元素进行某种特定的处理   (重点是特定的处理)
###### 迭代器模式中主要用于逐个遍历保存在数据结构中的一些元素   (重点是遍历)
    
#### 源码中的应用
    ava.nio.file.FileVisitor
    org.springframework.beans.factory.config.BeanDefinitionVisitor    
    
    
### 状态模式 
#### 定义：允许一个对象在其内部状态改变时，改变它的行为
#### 主要解决的是：当控制一个对象状态转换的过程比较复杂的时候，就可以把状态的判断逻辑转移到表示不同状态的一系列类中
#### 类型：行为型
...
#### 源码中的应用
    com.sun.faces  jsf-api   LifeCycle
